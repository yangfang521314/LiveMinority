 ...
 * Date: 16/04/24
 */
class SwipeMenu @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    private fun forceUniformHeight(count: Int, widthMeasureSpec: Int) {
        ...
        val uniformMeasureSpec = View.MeasureSpec.makeMeasureSpec(
            measuredHeight,
...
//                        //只要有一个侧滑菜单处于打开状态， 就不给外层布局上下滑动了
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    if (Math.abs(gap) > 5 || Math.abs(scrollX) > 5) {//2016 09 29 修改此处，使屏蔽父布局滑动更加灵敏，
                        parent.requestDisallowInterceptTouchEvent(true)
                    }
...
//                    //2016 10 22 add , 仿QQ，侧滑菜单展开时，点击内容区域，关闭侧滑菜单。begin
                        ...
                        if (Math.abs(velocityX) > 1000) {//滑动速度超过阈值
                            if (velocityX < -1000) {
                                if (isLeftSwipe) {//左滑
                                    //平滑展开Menu
                                    smoothExpand()
                                    menListener?.closeLive()


                                } else {
                                    //平滑关闭Menu
                                    LogUtil.e("1")
                                    menListener?.showLive()
                                    smoothClose()
    ...
    private var menListener: MenuListener? = null
    ...
    fun setOnMenuClickListener(listener: MenuListener) {
        menListener = listener
    }
